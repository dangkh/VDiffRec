...
parser.add_argument('--emb_path', type=str, default='../datasets/', help='load data path')
...
parser.add_argument('--dims', type=str, default='[64]', help='the dims for the DNN')
...
# param for Autoencoder
parser.add_argument('--lrae', type=float, default=0.05, help='hyper-parameter of multinomial log-likelihood for AE: 0.01, 0.02, 0.03, 0.05')
...
latent_size = 128
Autoencoder = AE(item_emb, latent_size, device).to(device)
...
### Build MLP ###
out_dims = eval(args.dims) + [latent_size]
in_dims = out_dims[::-1]
model = DNN(in_dims, out_dims, args.emb_size, time_type="cat", norm=args.norm).to(device)

optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
optimizer1 = optim.AdamW(Autoencoder.parameters(), lr=args.lrae, weight_decay=1e-5)
print("models ready.")
...
param_num = mlp_num + diff_num + AE_num
